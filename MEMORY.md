...
### ✅ 运行时行为校验 (v0.5) — Headless 自动化测试
- **开始日期**: 2025-02-14
- **核心架构**: 插件生成确定性测试脚本 + Godot `--headless -s` 子进程运行
- **流程**: 静态校验通过 → 写入 `TEST_HARNESS_BASE` 测试脚本 → headless 子进程运行 300 帧 → 读取结果 → 有错误则进入修复循环
- **测试脚本**: `extends SceneTree`，load 主场景 → instantiate → 运行 300 帧 → 输出 `_test_results.json`
- **超时机制**: macOS `timeout 15` / `gtimeout 15` 硬超时 + 脚本内 `max_frames` 自杀
- **隔离**: 子进程崩溃不影响编辑器，exit code + 结果文件双重检测
- **线程集成**: 复用 `worker_thread` 模式，`_start_runtime_test()` → `_run_runtime_test_threaded()` → `_on_runtime_test_complete()`
- **修复循环集成**: 运行时错误使用 `RUNTIME:` 前缀，直接进入现有 `REPAIR_PROMPT` 循环
- **关键函数**: `_detect_main_scene()`, `_write_test_harness()`, `_run_runtime_test()`, `_read_runtime_test_results()`, `_start_runtime_test()`, `_on_runtime_test_complete()`
- **状态**: Phase 1（崩溃检测）已实现，Phase 2（输入模拟 + 游戏类型断言）待后续
